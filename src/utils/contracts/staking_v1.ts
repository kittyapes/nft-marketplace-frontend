import { appProvider, appSigner, currentUserAddress, stakedHinataBalance, stakingWaifuRewards } from '$stores/wallet';
...
export const stakeTokens = async (tokensToStake: string, durationLockUp: number) => {
	try {
		...
		// Notify User
		notifySuccess(`Successfully staked ${tokensToStake} for ${daysFromNow(durationLockUp * 1000).days} days`);
