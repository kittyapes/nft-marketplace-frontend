...
export const whitelistCollection = async (address: string, slug: string) => {
	...
	let royalties = [{ fees: res.data.collection.dev_seller_fee_basis_points / 10, address: res.data.collection.payout_address }];
	...
	await contractCaller(factoryContract, 'register', 150, 1, address, beneficiaries, percentages).catch(() => notifyError('Failed to whitelist collection on-chain'));
