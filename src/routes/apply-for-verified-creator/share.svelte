<script lang="ts">
    ...
    export let bio = 'Lorem ipsum dolor sit amet, consectetur adipisci elit, sed do eiusmod tempor incidunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrum exercitationem ullamco laboriosam, nisi ut aliquid ex ea commodi consequatur.'
                ...
                <div class='grid w-11/12 h-full gap-x-6 place-items-center grid-cols-[1fr_3fr]'>
                    ...
                    <div class='flex flex-col gap-2 p-2 place-self-start'>
                        <div class='flex text-left gap-3 '>
                            ...
                            <VerifiedCheckmarkWhite></VerifiedCheckmarkWhite>
                        </div>
                        <p class='font-light text-sm max-w-[15rem]'>{bio}</p>
                    </div>
                </div>
                <div class='absolute -bottom-5 right-[13%] grid place-items-center' 
                class:right={moveRight} 
                use:outsideClickCallback={{ cb: () => (showShare = false) }}
                >
                    {#if showShare}
                        <div class='rounded-lg bg-white bg-opacity-80 py-5 px-10 flex text-black gap-3' transition:slide on:outroend={() => (moveRight = false)}>
                            <div class='icon' on:click={() => {/* twitter share */}}>
                                <Twitter {gradient}></Twitter>
                            </div>
                            <div class='icon' on:click={() => {/* facebook share */}}>
                                <Facebook {gradient}></Facebook>
                            </div>
                            <div class='icon' on:click={() => {/* instagram share */}}>
                                <Instagram {gradient}></Instagram>
                            </div>
                            <div class='icon' on:click={() => {/* web share */}}> 
                                <Web {gradient}></Web>
                            </div>
                        </div>
