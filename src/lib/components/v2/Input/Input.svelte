...
<div class="w-full flex flex-row items-center rounded-lg  outline-color-purple {$$props.class}" class:outline-red-400={!valid} style:height>
	<div><slot /></div>
	<input type="text" bind:value {placeholder} class:pl-0={noLeftPadding} class="w-full h-full px-4 bg-inherit outline-none" on:input={validate} {disabled} on:keydown />
</div>
