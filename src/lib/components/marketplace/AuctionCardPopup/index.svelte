		...
		<div class="w-full md:w-1/2 bg-white p-8 flex flex-col h-full">
				...
				<div in:fade={{ duration: 300 }} class="h-full flex flex-col justify-between">
					...
					<div class='flex gap-4 flex-col place-items-center'>
						...
						<div class='w-[95%] grid place-items-center relative'>
							<input bind:value={inputBid} type='number' class='w-full p-2 rounded-md border border-color-black' placeholder=00.00>
							<button class='rounded-md bg-color-black text-white font-semibold text-xl px-12 py-[7px] absolute right-0'>Ξ</button>
						</div>
			...
			{:else}
				...
				<div class="py-5 flex-grow overflow-hidden">
					{#if tab == 0}
						<CardInfoTab on:click={() => bidScreen = true}/>
					{:else if tab == 1}
						<CardBidsTab on:click={() => bidScreen = true} />
					{:else if tab == 2}
						<CardHistoryTab />
					{/if}
				</div>
