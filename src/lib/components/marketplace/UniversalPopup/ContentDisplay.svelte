...
{#if buyScreen}
	<div in:fade={{ duration: 300 }} class="h-full flex flex-col justify-between mb-5">
		...
		<slot name="buy-screen" />
		<slot name="buy-screen-footer" />
	</div>
{:else if successScreen}
	<div in:fade={{ duration: 300 }} class="h-full flex flex-col justify-between">
		...
		<slot name="success-screen" />
	</div>
{:else}
	<!-- Tabs container -->
	<div class="w-full flex items-center justify-between">
		<!-- Tabs -->
		<TabSwitcher bind:selectedTab={tab}><slot name="mid-tab-nav-content" slot="mid-tab-nav" /></TabSwitcher>
		<CloseButton />
	</div>
	...
	<div class="py-5 flex-grow overflow-hidden">
		<div class="w-full h-full flex flex-col">
			<div class="h-full flex flex-col justify-between">
				{#if tab == 0}
					<slot name="first-tab" />
					<slot name="first-tab-footer" />
				{:else if tab == 1}
					<slot name="mid-tab" />
					<slot name="mid-tab-footer" />
				{:else if tab == 2}
					<slot name="last-tab"><CardHistoryTab /></slot>
				{/if}
			</div>
		</div>
	</div>
