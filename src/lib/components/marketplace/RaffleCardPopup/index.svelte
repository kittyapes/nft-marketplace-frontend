<script lang='ts'>
    ...
    export let rowInfo: PopupRowInfo[] = Array(20).fill(0).map((_, i) => ({id: i, message: '', nickname: 'Philip', imageUrl: '', amount: 10} as PopupRowInfo))
    ...
    let buttonClick = () => {
        if(inputBidValue) {
            if(!isNaN(inputBidValue) && inputBidValue >= ticketPrice) {
                ...
                inputBid = `${Math.round((Number(inputBid) - (Number(inputBid) % ticketPrice) + Number.EPSILON) * 100) / 100} Ξ`;
    ...
    <ContentDisplay slot='content' bind:buyScreen={buyScreen} bind:successScreen={successScreen}>
        ...
        <CardBuyScreen slot='buy-screen' currencySymbol={'Ξ'}>
            <div class='w-[95%] grid place-items-center relative' slot='buy-input'>
                ...
                <button class='rounded-md bg-color-black text-white font-semibold text-xl px-[12%] absolute right-0 top-0 bottom-0' on:click={buttonClick} >
        ...
        <TabFooter slot='buy-screen-footer'
            ...
            buttonText={'BUY'}
            infoText={[{'Potential odds': potentialOdds}, {'Drawing in': endingIn.getDate() - 1 + 'D ' + endingIn.getHours() + 'H ' + endingIn.getMinutes() + 'M'}]}
            {buyButtonDisabled} 
        />

        <SuccessScreen slot='success-screen' {successMessage}></SuccessScreen>
        ...
        <div slot='mid-tab-row'>
            ...
            {#each rowInfo as row}
			    <EntriesTabRow {row} odds={Math.round(((100 / (currentEntries / row.amount)) + Number.EPSILON) * 100) / 100}/>
	    	{/each}
        </div>
        </CardMidTab>
        <TabFooter slot='mid-tab-footer' on:click={() => {successMessage = 'You have purchased and entered raffle tickets'; successScreen = false; buyScreen = true;}}
