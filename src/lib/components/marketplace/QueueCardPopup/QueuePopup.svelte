<script lang="ts">
	...
	export let rowInfo: PopupRowInfo[] = Array(20)
		...
		.map(
			(_, i) =>
				({ id: i, message: 'Ξ', nickname: 'Mitchell', imageUrl: '', amount: 1.2 } as PopupRowInfo)
		);

	export let queueEntry = false;

	let infoText: { [key: string]: any }[] = [
		{ 'Entry requirements': `${entryReq} WAIFU` },
		{ 'Users in pre-queue': queueTotal }
	];
	...
	$: if (queueEntry) {
		infoText = [
			{ 'NFTs remainings': remainingNfts },
			{
				'Claim queue is live for':
...
<PopupContainer>
	<ContentDisplay slot="content" bind:buyScreen bind:successScreen bind:tab>
		<SuccessScreen slot="success-screen" successMessage={'Your have cut the queue!'} />

		<button
			slot="success-screen-back-bt"
		...
		>
			<div slot="bt" class="w-full">
				{#if queueEntry}
					<QueueCountDisplay currentVal={queueCurrent} totalVal={queueTotal} />
				{:else}
					<button class="btn btn-gradient btn-rounded w-full" disabled={false}>
						{`ENTER PRE-QUEUE`}
					</button>
				{/if}
			</div>
		...
		<CardMidTab slot="mid-tab" crossOld={true} {rowInfo}>
			<div slot="mid-tab-row">
				...
				{#each rowInfo as row}
					<QueueCardRow {row} out={row.id > remainingNfts} />
				{/each}
			</div></CardMidTab
		>
		<TabFooter
