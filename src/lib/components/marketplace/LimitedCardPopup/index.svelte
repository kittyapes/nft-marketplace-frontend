<script lang='ts'>
    ...
    export let rowInfo: PopupRowInfo[] = Array(20).fill(0).map((_, i) => ({id: i, message: '35 min', nickname: 'Philip', imageUrl: '', amount: undefined} as PopupRowInfo))
    ...
    let buttonClick = () => {
        if(inputBid) {   
            if(Number(inputBid) != NaN && Number(inputBid) != 0) {
...
<PopupContainer>
    <ContentDisplay slot='content' bind:buyScreen={buyScreen} bind:successScreen={successScreen}>
        ...
        <SuccessScreen slot='success-screen' successMessage={'Your have claimed your limited NFT'}></SuccessScreen>
        ...
        <TabFooter slot='first-tab-footer' on:click={() => {successScreen = false; buyScreen = true;}}
            ...
            infoText={[{'NFT Remaining': remainingNfts}, {'Ending in': endingIn.getDate() - 1 + 'D ' + endingIn.getHours() + 'H ' + endingIn.getMinutes() + 'M'}]}>
        </TabFooter>

        <CardMidTab slot='mid-tab' crossOld={false} {rowInfo} bold={true}></CardMidTab>
        <TabFooter slot='mid-tab-footer' on:click={() => {successScreen = false; buyScreen = true;}}
            buttonText={`BUY FOR ${nftCost} Hi`}
            infoText={[{'Owners': rowInfo.length}, {'Ending in': endingIn.getDate() - 1 + 'D ' + endingIn.getHours() + 'H ' + endingIn.getMinutes() + 'M'}]}>
        </TabFooter>
